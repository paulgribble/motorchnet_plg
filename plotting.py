...
def plot_activation(all_hidden, all_muscles, xy, tg):
    fg, ax = plt.subplots(nrows=8,ncols=3,figsize=(10,20))

    x = np.linspace(0, 1, 100)

    for i in range(8):
        ax[i,0].plot(x,np.array(tg[i,:,:]),'--')
        ax[i,0].plot(x,np.array(xy[i,:,:]))
        ax[i,1].plot(x,np.array(all_muscles[i,:,:]))
        ax[i,2].plot(x,np.array(all_hidden[i,:,:]))
        
        ax[i,0].set_ylabel('Cartesian (m)')
        ax[i,1].set_ylabel('muscle act (au)')
        ax[i,2].set_ylabel('hidden act (au)')
